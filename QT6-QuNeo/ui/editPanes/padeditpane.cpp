...
//    this->setStyleSheet("QLabel { font: 10px }");
    ...
    qDebug() << "velTableMenu connect - name: " << velTableMenu->objectName();
...
void PadEditPane::findUiElements()
{
    ...
    //install event filter in all the spinboxes so that
    //copy/paste commands can be intercepted while they are in focus
    for(int i=0; i<padParamList.count();i++)
       {
           if(spinBoxMap.contains(padParamList.at(i)))
           {
               spinBoxMap.value(QString(padParamList.at(i)))->installEventFilter(this);
           }
       }
    ...
    for(int i =0; i<4; i++)
    {
        ...
        backTransposeSpinBoxes[i]->installEventFilter(this);
    }

    ledDrumInChannel->installEventFilter(this);
    ledGridDiscreteInChannel->installEventFilter(this);
    ledGridDualInChannel->installEventFilter(this);
    cornerIsolation->installEventFilter(this);
    onThresh->installEventFilter(this);
    offThresh->installEventFilter(this);
    sensitivityDial->installEventFilter(this);
}
...
void PadEditPane::displayPadValues()
{
    ...
    // EB - I'm not certain why this gets blacklisted, but we want to recall the setting here
...
{
    qDebug() << "slotEvents called - string: " << string;
...
bool PadEditPane::eventFilter(QObject *object, QEvent *event){
    ...
    if(event->type() == QEvent::KeyPress){
       ...
       if(keyEvent->matches(QKeySequence::Copy)){
           emit signalTriggerCopy();
       }
       if(keyEvent->matches(QKeySequence::Paste)){
           emit signalTriggerPaste();
       }
    }
    return 0;
}
...
{
    qDebug()<<QString("slotPadSensitivityExtreme called");

    int val_globalSensitivity = 127;
    int val_perPadSensitivity = 20;
    int val_onThreshold = 3;
...
void PadEditPane::slotSetPadSensitivity(int val_globalSensitivity, int val_perPadSensitivity, int val_onThreshold, int val_offThreshold, QString val_velTable)
{
    ...
    onThresh->setValue(val_onThreshold);
    emit signalValueChanged(currentPreset, "ComponentSettings", "Pad", currentPad, QString("padOnThreshold"),val_onThreshold);
    ...
    for(int i =0; i<16; i++)
    {
        emit signalCurrentSensor(QString("padButton%1").arg(i));
        perPadSensitivityUI->setValue(val_perPadSensitivity);
        emit signalValueChanged(currentPreset, "ComponentSettings", "Pad", i, QString("padSensitivityPerPadUI"), val_perPadSensitivity);
        displayPadValues();
        emit signalCurrentSensor(QString("padButton%1").arg(i));
        padLabel[i]->slotDisplayParameter(QString("padSensitivityPerPadUI"), i, currentPreset);
    }
